var _ = require('lodash'),
    ...
    { AuthLoader } = require('postman-runtime/lib/authorizer'),
  ...
  initialize () {
    pm.eventBus.channel('postman-runtime').subscribe(function (message) {
      ...
      if (message.name === 'previewRequest') {
        let requestToPreview = new postmanCollectionSdk.Request(message.data && message.data.request),
          info = (message.data && message.data.info) || {};

        let dryRunOptions = {
          implicitCacheControl: info.implicitCacheControl,
          implicitTraceHeader: info.implicitTraceHeader,
          protocolProfileBehavior: info.protocolProfileBehavior,
          cookieJar: info.cookiePartitionId ?
            new CookieJar(info.cookiePartitionId, { readFromDB: true, writeToDB: false }) : null
        };

        try {
          dryRunRequest(requestToPreview, dryRunOptions, (err, previewedRequest) => {
            if (err) {
              return pm.eventBus.channel('postman-runtime').publish({

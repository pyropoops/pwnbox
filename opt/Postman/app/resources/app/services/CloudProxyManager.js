...
class CloudProxyManager {
  ...
  init (cb) {
    FileService.read(CONFIG_FILE_PATH)
      ...
      // If there is any error while reading the proxy config from the file, we catch it and fallback to
      // the DEFAULT_PROXY_CONFIG.
      .catch((e) => {
        pm.logger.warn('CloudProxyManager~init: Error while reading proxy configuration from user data - ', e);
  ...
  attachProxyEventListeners () {
    ipcMain.on('handleProxyAuthSubmit', (event, credentials = {}) => {
    ...
    ipcMain.on('handleProxyAuthCancel', (event) => {
